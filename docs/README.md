...
| **[`4_API_AND_DATA_SCHEMAS.md`](./4_API_AND_DATA_SCHEMAS.md)** | **Giao Kèo API & Kiểu Dữ Liệu** | Chuẩn RESTful API (Auth, Lessons, Progress), WebSocket Realtime, Error responses chuẩn hóa, và gói kiểu TypeScript `@meowshadow/types`. |
| **[`5_SPRINT_PLAN.md`](./5_SPRINT_PLAN.md)** | **Kế Hoạch Triển Khai 5 Sprint** | Lộ trình chi tiết từng Sprint từ Docker nền tảng, Audio Processor, AI Workers, Go Gateway, Next.js Web đến React Native Mobile App. |
...
Dự án hiện đã hoàn tất **Giai đoạn Thiết kế & Phê duyệt Kiến trúc (Phase 0)** và đang bước vào **Sprint 1: Nền tảng Docker, Shared Types & Audio Processor Worker**.
...
### 2. Mục tiêu bàn giao của Sprint 1
Sau khi Sprint 1 hoàn thành dựng khung Monorepo và `docker-compose.yml`, bạn sẽ khởi chạy toàn bộ hệ sinh thái với lệnh duy nhất:
```bash
# Start PostgreSQL, Redis, Shared Storage, and Audio Processor Worker
...
* 🌐 **Web Studio UI (Sprint 4):** `http://localhost:3000`
* 🚀 **Golang Gateway API & Docs (Sprint 3):** `http://localhost:8000/swagger/index.html`
* 🗄️ **PostgreSQL Server (Sprint 1):** `localhost:5432` (DB: `meowshadow_db`)
* ⚡ **Redis Broker (Sprint 1):** `localhost:6379`
